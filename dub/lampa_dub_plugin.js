...
(function () {
    ...
    DubController.prototype.updateDucking = function () {
        ...
        // Во время приглушения ИЛИ пока едет rampVolume() — video.volume
        // временно не соответствует реальному пользовательскому значению
        // (см. isVolumeRamping), читать его в этот момент как "новую
        // громкость от пользователя" нельзя — иначе положительная обратная
        // связь садит звук всё тише с каждой репликой.
        if (!shouldDuck && !isVolumeRamping()) {
